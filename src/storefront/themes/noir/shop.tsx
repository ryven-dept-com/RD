import Link from "next/link";
import {
  MobileFilters,
  SearchBar,
  SortSelect,
} from "@/app/(store)/shop/shop-controls";
import type { ShopProps } from "@/storefront/types";
import { NoirCard } from "./card";

/**
 * NOIR DEPT shop — no sidebar: a dramatic dark header, a single toolbar
 * (filters drawer + search + sort) and a spacious two-column editorial grid.
 */
export function NoirShop({ data, tr, fmt }: ShopProps) {
  const { products, options, heading, categoryDescription } = data;

  return (
    <div className="rd-needs-offset bg-bone pt-16">
      {/* dramatic header band */}
      <div className="border-b border-brand-100 bg-brand-50">
        <div className="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8">
          <nav className="mb-4 flex items-center gap-2 text-xs uppercase tracking-[0.3em] text-brand-400">
            <Link href="/" className="hover:text-ink">
              {tr("shop.home")}
            </Link>
            <span className="text-amber">—</span>
            <span className="text-ink">{heading}</span>
          </nav>
          <h1 className="font-display text-5xl uppercase tracking-tight sm:text-7xl">
            {heading}
          </h1>
          <p className="mt-3 max-w-xl text-sm text-brand-400">
            {tr("shop.subtitle", { count: products.length })}
          </p>
          {categoryDescription && (
            <p className="mt-3 max-w-2xl text-sm leading-relaxed text-brand-500">
              {categoryDescription}
            </p>
          )}
        </div>
      </div>

      <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
        {/* single toolbar */}
        <div className="mb-10 flex flex-wrap items-center justify-between gap-4 border-b border-brand-100 pb-6">
          <div className="flex w-full flex-wrap items-center gap-3 sm:w-auto sm:flex-1">
            <MobileFilters options={options} />
            <SearchBar className="max-w-md flex-1" />
          </div>
          <SortSelect resultCount={products.length} />
        </div>

        {products.length === 0 ? (
          <div className="flex flex-col items-center justify-center gap-4 rounded-md border border-brand-100 py-28 text-center">
            <p className="font-display text-4xl uppercase tracking-wide">{tr("shop.nothingHere")}</p>
            <p className="max-w-sm text-sm text-brand-400">{tr("shop.noResults")}</p>
            <Link
              href="/shop"
              className="rd-cta rounded-full bg-ink px-6 py-3 text-xs font-semibold uppercase tracking-widest text-bone"
            >
              {tr("shop.viewAll")}
            </Link>
          </div>
        ) : (
          <div className="grid grid-cols-1 gap-x-5 gap-y-12 sm:grid-cols-2">
            {products.map((p, i) => (
              <NoirCard key={p.slug} product={p} tr={tr} fmt={fmt} index={i} priority={i < 2} />
            ))}
          </div>
        )}
      </div>
    </div>
  );
}
